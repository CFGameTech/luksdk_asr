...
- (void)onGameStateChangeState:(NSString *)state dataJson:(NSString *)dataJson{
    
    if ([state isEqualToString:cf_game_call_asr]) {
        
        NSLog(@"cf_game_call_asr");
        NSArray *array = @[@"zh_CN",@"en"];
        [CFGameSDK gameASRInitWithLanguages:array success:^{
            
            NSLog(@"cf_game_call_asr success");
            
        } fail:^(NSString * _Nonnull msg) {
            NSLog(@"cf_game_call_asr fail");
        }];
